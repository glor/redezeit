...
function updateTable(state) {
    ...
    for (var key in state.registered_keys) {
        const seconds = state.registered_keys[key].count;
        $("#seconds_" + key).html(`${Math.floor(seconds / 60)}m ${seconds % 60}s`);
...
function State() {
    ...
    this.current_key = "-";
    this.registered_keys["-"] = {};
    this.registered_keys["-"].name = "Moderation";
...
$().ready(function () {
        ...
        $("#resettimes").click(function () {
            state.history = new Array();
            for (var key in state.registered_keys) {
                state.registered_keys[key].count = 0;
            }
            state2table(state);
            updateTable(state);
            saveState(state);
